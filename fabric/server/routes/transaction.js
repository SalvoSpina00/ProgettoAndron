...
router.post('/', gatewayConnectionAndron, async (req, res) => {
    try {
        let result=Buffer.from(await contract.submitTransaction(`transaction:createTransaction`,JSON.stringify(req.body))).toString()
        res.status(result.status==="error" ? 400 : 200).json(JSON.parse(result));
    } 
    catch (error) {
